...
use args::{
    agent_pack_query, company_report_query, daily_query, document_query, document_read_query,
    efts_query, eightk_exhibit_query, eightk_query, fact_query, filing_query, foreign_query,
    form4_query, fund_query, health_score_query, ixbrl_query, metrics_query, parse_query,
    prospectus_query, proxy_query, report_kind, report_query, schedule13_query, search_inputs,
    section_query, statement_query, stitch_query, table_query, thirteenf_query,
};
...
async fn call_tool(client: &SecClient, params: Value) -> Result<Value> {
    ...
    let data = match name {
        ...
        "sec_agent_pack" => json!(
            client
                .agent_pack(agent_pack_query(client, &args).await?)
                .await?
        ),
        "sec_ixbrl" => json!(
            client
